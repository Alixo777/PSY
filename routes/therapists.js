...
const {therapistsModel, validatePatient, validLogin, createToken } = require('../models/therapistsModel'); // Or whatever the correct name is
...
router.post('/register', async (req, res) => {
  ...
  try {
      ...
      const existingPatient = await therapistsModel.findOne({ email });
      if (existingPatient) {
      ...
      const newPatient = new therapistsModel({
        tCode,
        firstName,
        lastName,
        img,
...
router.post("/login", async (req, res) => {
  ...
  try {
      ...
      const patient = await therapistsModel.findOne({ email });
      if (!patient) {
...
router.get("/myInfo", async (req, res) => {
    ...
    try {
      ...
      // 
      let patient = await therapistsModel.findOne({ _id: tokenData._id },
         { password: 0 });
...
router.put("/:id", async (req, res) => {
    ...
    try {
        ...
        let updatedPatient = await therapistsModel.findByIdAndUpdate(
            patientId,
            req.body,
